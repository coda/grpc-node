        ...
        http2Server.on('session', (session) => {
            ...
            if ('grpc.keepalive_time_ms' in this.options) {
                ...
                session.setTimeout(this.options['grpc.keepalive_time_ms'], () => {
                    ...
                    logging.log(constants_1.LogVerbosity.INFO, `Sending session keepalive ping after ${this.options['grpc.keepalive_time_ms']}ms idle`);
                    ...
                    const pingWatchdog = setTimeout(() => {
                        if (!pingSuccess) {
                            ...
                            logging.log(constants_1.LogVerbosity.ERROR, `Closing http session because no ping response heard after ${pingTimeoutMs}ms.`);
                            session.close();
                        }
                    }, pingTimeoutMs);
                    const wasPingSent = session.ping((err, duration) => {
                        ...
                        // TODO: What verbosity is appropriate?
                        logging.log(constants_1.LogVerbosity.ERROR, `Error on keepalive ping: [name]:${err.name} [message]:${err.message} [stack]:${err.stack}`);
                    ...
                    if (!wasPingSent) {
                        ...
                        logging.log(constants_1.LogVerbosity.ERROR, `Failed to send keepalive ping.`);
                    }
                });
            }
            session.on('close', () => {
