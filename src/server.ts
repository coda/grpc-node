...
export class Server {
  ...
  constructor(options?: ChannelOptions) {
    ...
    console.log('GRPCGRPC - server constructor - ' + options?.name);
    console.log(options);
  }
  ...
  private _setupHandlers(
    ...
    http2Server.on('session', (session) => {
      ...
      function tmpLog(s: string) {
        console.log(`GRPCGRPC ${options?.name} session ${sessionId}: ${s}`);
        logging.log(LogVerbosity.ERROR, s);
      }

      const keepaliveTimeMs = this.options['grpc.keepalive_time_ms'];
      tmpLog('new session - keepalive_time_ms: '+keepaliveTimeMs);
      if (keepaliveTimeMs) {
